...
//    			System.out.println("文件存在");
    ...
    public static void processBody(String body,String from,List<String> dataAdd, String time) {
    	SimpleDateFormat sdf = new SimpleDateFormat("yy-MM-dd HH:mm:ss");
    	String s = "";
    	if(body.contains("weight")) {
    		String weight = body.substring(body.indexOf("weight") + 7, body.indexOf(","));
    		String path =  GlobalConstants.path + from + " " + GlobalConstants.type + ".csv";
    		List<String> datas = new ArrayList<String>();
			datas.add(weight);
			datas.add(GlobalConstants.type);
			if(dataAdd != null && dataAdd.size() > 0) {
				for(int i=0;i<dataAdd.size();i++) {
					datas.add(dataAdd.get(i));
				}
			}
    		WriteCSV.writeNewFile(path);
    		WriteCSV.writeData(path, datas,time);
